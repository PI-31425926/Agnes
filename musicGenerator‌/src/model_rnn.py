...
架构：
  Embedding -> RNN(x2) -> Linear -> Softmax

最基础的循环神经网络，参数量最少，训练最快。
...
class MusicRNN(nn.Module):
    ...
    def generate(
        ...
        output_mode = getattr(self, 'output_mode', 'text')

        # For MIDI mode, EOS is not a valid stopping signal (training data has 0 EOS)
        if output_mode == 'midi':
            eos_idx = -1  # disable EOS check
        else:
    ...
    def load(self, filepath: str):
        ...
        self.model_type = checkpoint.get('model_type', 'rnn')
